...
state_dict = torch.load(os.path.join('checkpoints', opt.warp_checkpoint), map_location=torch.device('cpu'))
state_dict.pop('aflow_net.weight')
warp_model.load_state_dict(state_dict)
warp_model.to(device)
warp_model.eval()

# Load onnx model
gen_model = onnxruntime.InferenceSession("checkpoints/onnx/gen_QAT_Pytorch.onnx")
...
for i, data in enumerate(dataset, 0):
    ...
    inputs = {gen_model.get_inputs()[0].name: gen_inputs.detach().numpy()}
    gen_outputs = gen_model.run(None, inputs)
    ...
    FilePath = 'result/Tryon_GenQAT_' + opt.warp_checkpoint[-7:-4]
