...
# Author: Zhihao Li
# Time: July 20, 2023
# Function: This is used for pruning the Gen Model
#------------------Programe Overview-----------------
...
#---------------------------Load WarpModel Prepruning Subnetwork-------------------
IFPN_Module = IFPN()
CFPN_Module = CFPN(3)
...
warp_state_dict = torch.load(opt.PFAFN_warp_checkpoint, map_location=torch.device('cpu'))
...
PF_warp_model.load_state_dict(warp_state_dict['model'])
PF_warp_model.cuda()
...
ignored_layers = []
ignored_layers.append(PF_gen_model.model.model[6])

# 2. Pruner initialization
print("Start initializing pruner...")
example_inputs = torch.randn(1, 7, 256, 192).to(device)
sparsity = 0.2
iterative_steps = 3 # prune model iteratively.
Gen_pruner = tp.pruner.MagnitudePruner(
    PF_gen_model, 
    example_inputs, 
...
print("Start pruning gen_model...")
base_macs, base_nparams = tp.utils.count_ops_and_params(PF_gen_model, example_inputs)
...
for i in range(iterative_steps):
    ...
    Gen_macs, Gen_nparams = tp.utils.count_ops_and_params(PF_gen_model, example_inputs)
    print(
        "  Iter %d/%d, Params: %.2f M => %.2f M"
        % (i+1, iterative_steps, base_nparams / 1e6, Gen_nparams / 1e6)
    )
    print(
        "  Iter %d/%d, MACs: %.2f G => %.2f G"
        % (i+1, iterative_steps, base_macs / 1e9, Gen_macs / 1e9)
    )
...
# save pruned and finetuned models
warp_state_dict['model'] = PF_warp_model.state_dict()
gen_state_dict = {
    'model': PF_gen_model.state_dict(),
    'pruning': Gen_pruner.pruning_history(),
}
torch.save(warp_state_dict, os.path.join(FilePath, 'WarpModel_Pruned_' + str(sparsity) + '.pth'))
torch.save(gen_state_dict, os.path.join(FilePath, 'GenModel_Pruned_' + str(sparsity) + '.pth'))
print("Successfully pruned the models!")
