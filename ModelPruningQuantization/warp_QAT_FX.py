...
import torch.ao.quantization.prepare_fx as prepare_fx
import os
import numpy as np
import torch
import time
...
qconfig_mapping =QConfigMapping.set_global('qnnpack')
PF_warp_model.train()
example_inputs = (torch.randn(1, 3, 256, 192), torch.randn(1, 3, 256, 192))
prepare_custom_config_dict = {
    "non_traceable_module_name": "non_traceable_submodule",
}
quantized_model = prepare_fx(PF_warp_model, 
                                             qconfig_mapping,
                                             example_inputs,
...
PF_warp_model.eval()
# Fuse the model in place rather automatically.
PF_warp_model = quantize_fx.fuse_fx(PF_warp_model)
...
# optimizer
params = [p for p in quantized_model.parameters()]
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    if epoch > opt.niter:
        quantized_model.update_learning_rate(optimizer)
