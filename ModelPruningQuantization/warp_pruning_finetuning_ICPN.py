...
# Author: Zhihao Li
# Time: July 15, 2023
# Function: This is used for pruning the Image Features Encoder Module, 
#           Cond Features Encoder Module, Features Pyramid Network Module in Warp_Model
#------------------Programe Overview-----------------
...
from models.afwm import TVLoss, AFWM, IFPN, CFPN
...
IFPN_Module.image_features, CFPN_Module.cond_features = PF_warp_model.image_features, PF_warp_model.cond_features
IFPN_Module.image_FPN, CFPN_Module.cond_FPN = PF_warp_model.image_FPN, PF_warp_model.cond_FPN
IFPN_Module.eval()
IFPN_Module.cuda()
CFPN_Module.eval()
CFPN_Module.cuda()
...
#--------------------------Prune Warp_model.IFPN, Warp_model.CFPN-----------------------
example_inputs = torch.randn(1, 3, 256, 192).to(device)
# original module
IFPN_origin_output, CFPN_origin_output = IFPN_Module(example_inputs), CFPN_Module(example_inputs)
torch.save(IFPN_Module.state_dict, os.path.join(FilePath, 'Original_IFPN.pth'))
torch.save(CFPN_Module.state_dict, os.path.join(FilePath, 'Original_CFPN.pth'))
...
IFPN_ignored_layers, CFPN_ignored_layers = [], []
for i in range(5):
    IFPN_ignored_layers.append(IFPN_Module.image_FPN.smooth[i])
    CFPN_ignored_layers.append(CFPN_Module.cond_FPN.smooth[i])

# 2. Pruner initialization
print("Start initializing pruner...")
sparsity = 0.5
iterative_steps = 5 # prune model iteratively.
IFPN_pruner = tp.pruner.MagnitudePruner(
    IFPN_Module, 
    example_inputs, 
    ...
    ignored_layers=IFPN_ignored_layers,
)
CFPN_pruner = tp.pruner.MagnitudePruner(
    CFPN_Module, 
    example_inputs, 
    global_pruning=False, # If False, a uniform sparsity will be assigned to different layers.
    importance=imp, # importance criterion for parameter selection
    iterative_steps=iterative_steps, # the number of iterations to achieve target sparsity
    ch_sparsity=sparsity,
    ignored_layers=CFPN_ignored_layers,
)

print("Start pruning warp_model...")
IFPN_base_macs, IFPN_base_nparams = tp.utils.count_ops_and_params(IFPN_Module, example_inputs)
CFPN_base_macs, CFPN_base_nparams = tp.utils.count_ops_and_params(CFPN_Module, example_inputs)
for i in range(iterative_steps):
    ...
    IFPN_macs, IFPN_nparams = tp.utils.count_ops_and_params(IFPN_Module, example_inputs)
    CFPN_macs, CFPN_nparams = tp.utils.count_ops_and_params(CFPN_Module, example_inputs)
    print(
        "  Iter %d/%d, IFPN Params: %.2f M => %.2f M; CFPN Params: %.2f M => %.2f M"
        % (i+1, iterative_steps, IFPN_base_nparams / 1e6, IFPN_nparams / 1e6, 
           CFPN_base_nparams / 1e6, CFPN_nparams / 1e6)
    )
    print(
        "  Iter %d/%d, IFPN MACs: %.2f G => %.2f G; CFPN MACs: %.2f G => %.2f G"
        % (i+1, iterative_steps, IFPN_base_macs / 1e9, IFPN_macs / 1e9, 
           CFPN_base_macs / 1e9, CFPN_macs / 1e9)
    )
...
# Validation
IFPN_prune_output, CFPN_prune_output = IFPN_Module(example_inputs), CFPN_Module(example_inputs)
IFPN_state_dict = {
    'warp_pruned_model': IFPN_Module.state_dict(),
    'pruning': IFPN_pruner.pruning_history(),
}
CFPN_state_dict = {
    'warp_pruned_model': CFPN_Module.state_dict(),
    'pruning': CFPN_pruner.pruning_history(),
}
torch.save(IFPN_state_dict, os.path.join(FilePath, 'Pruned_IFPN_' + str(sparsity) + '.pth'))
torch.save(CFPN_state_dict, os.path.join(FilePath, 'Pruned_CFPN_' + str(sparsity) + '.pth'))

# Calculate accuracy
for i, j in zip(IFPN_origin_output, IFPN_prune_output):
    print("Channels change in every layer: {} ===> {}".format(i.shape, j.shape))
AAE = torch.mean(torch.abs(IFPN_origin_output[-1] - IFPN_prune_output[-1]))
ARE = AAE / torch.mean(torch.abs(IFPN_origin_output[-1]))
print("IFPN_Module Prune Average Absolute Error:{}, Average Relative Error: {}%".format(AAE, ARE*100))

for i, j in zip(CFPN_origin_output, CFPN_prune_output):
    print("Channels change in every layer: {} ===> {}".format(i.shape, j.shape))
AAE = torch.mean(torch.abs(CFPN_origin_output[-1] - CFPN_prune_output[-1]))
ARE = AAE / torch.mean(torch.abs(CFPN_origin_output[-1]))
print("CFPN_Module Prune Average Absolute Error:{}, Average Relative Error: {}%".format(AAE, ARE*100))

state_dict = {
    'model': PF_warp_model.state_dict(),
    'IFPN_pruning': IFPN_pruner.pruning_history(),
    'CFPN_pruning': CFPN_pruner.pruning_history(),
}
torch.save(state_dict, os.path.join(FilePath, 'WarpModel_ICPN_' + str(sparsity) + '.pth'))
