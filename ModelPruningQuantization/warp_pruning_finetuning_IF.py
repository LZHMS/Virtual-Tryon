...
# Author: Zhihao Li
# Time: July 15, 2023
# Function: This is used for pruning the Image Features Encoder Module in Warp_Model
#------------------Programe Overview-----------------
...
#--------------------------Prune Warp_model.image_features-----------------------
ModuleName = 'IF_module'
Module = PF_warp_model.image_features
example_inputs=torch.randn(1, 3, 256, 192).to(device)
# original module
origin_output = Module(example_inputs)
torch.save(Module.state_dict, os.path.join(FilePath, 'Original_' + ModuleName + '.pth'))
...
ignored_layers = []
for i in range(5):
    ignored_layers.append(Module.encoders[i][2].block[5])
...
base_macs, base_nparams = tp.utils.count_ops_and_params(Module, example_inputs)
for i in range(iterative_steps):
    ...
    macs, nparams = tp.utils.count_ops_and_params(Module, example_inputs)
    print(
    ...
    # finetune warp_model
    PF_warp_model.image_features = Module
    PF_warp_model.train()
...
prune_output = Module(example_inputs)
state_dict = {
    'warp_pruned_model': Module.state_dict(),
    'pruning': pruner.pruning_history(),
}
torch.save(state_dict, os.path.join(FilePath, 'Pruned_' + ModuleName + '_' + str(sparsity) + '.pth'))
# calculate accuracy
for i, j in zip(origin_output, prune_output):
    print("Channels change in every layer: {} ===> {}".format(i.shape, j.shape))
AAE = torch.mean(torch.abs(origin_output[-1] - prune_output[-1]))
ARE = AAE / torch.mean(torch.abs(origin_output[-1]))
print("Module Prune Average Absolute Error:{}, Average Relative Error: {}%".format(AAE, ARE*100))

state_dict = {
    'model': PF_warp_model.state_dict(),
    'pruning': pruner.pruning_history(),
}
torch.save(state_dict, os.path.join(FilePath, 'WarpModel_' + ModuleName + '_' + str(sparsity) + '.pth'))
