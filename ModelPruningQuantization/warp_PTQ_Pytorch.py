...
# Author: Zhihao Li
# Time: July 15, 2023
# Quantization Version: Pytorch Static Quantization
...
def validate(val_loader, model):
    ...
    for data in enumerate(val_loader):
        ...
        flow_out = model(p_tryon_un.detach(), clothes.cuda(), pre_clothes_edge.cuda())
...
fused_model = copy.deepcopy(PF_warp_model)
PF_warp_model.eval()
fused_model.eval()
...
for i in range(5):
    for j in range(1, 3):
        ...
        for k in range(2, 5):
            ...
            AFnet.append("aflow_net.netMain[" + str(i) + "][" + str(j) + '].block[' + str(k) + ']')
            AFnet.append("aflow_net.netRefine[" + str(i) + "][" + str(j) + '].block[' + str(k) + ']')
        FuseModules.append(IFLayer)
        FuseModules.append(CFLayer)
fused_model = torch.quantization.fuse_modules(fused_model, FuseModules, inplace=True)

AFnetMain, AFnetRefine = [], []
for k in range(2, 5):
    IFLayer.append("image_features.encoders[" + str(i) + "][" + str(j) + '].block[' + str(k) + ']')
    CFLayer.append("cond_features.encoders[" + str(i) + "][" + str(j) + '].block[' + str(k) + ']')
    AFnet.append("aflow_net.netMain[" + str(i) + "][" + str(j) + '].block[' + str(k) + ']')
...
# set config file for model quantization
conf = PostTrainingQuantConfig(approach="static")
q_model = quantization.fit(model=PF_warp_model, 
                           conf=conf, 
                           calib_dataloader=train_loader,
                           eval_func=validate)
q_model.save(FilePath)