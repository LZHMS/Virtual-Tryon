...
'''
--------------Program Description-------------
Version: Img2Col Algorithm(Store by Channel Size)
Author: Zhihao Li
Function: Achieve Correlation Module Acceleration
'''
...
class CorrTorch(nn.Module):
    ...
    def forward(self, fmap0: torch.Tensor, fmap1: torch.Tensor):
        # expand tensor fmap0
        fmap0 = fmap0.permute(0, 2, 3, 1).detach().cpu().numpy()
        N0, H0, W0, C0 = fmap0.shape
        fmap0 = fmap0.reshape(N0, -1)
        # padding fmap1 and expand tensor fmap1_pad
        fmap1_pad = F.pad(fmap1, self.pad_size, "constant", 0.0)
        fmap1_pad = fmap1_pad.permute(0, 2, 3, 1).detach().cpu().numpy()
        ...
        return torch.from_numpy(corr.reshape(N0, -1, H0, W0)).float().cuda()