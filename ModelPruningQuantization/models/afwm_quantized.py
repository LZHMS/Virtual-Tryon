...
from .correlation.corr_pure_torch import CorrTorch    # the custom cost volume layer
    ...
    def forward(self, x):
        ...
        for i, conv_ftr in enumerate(list(reversed(conv_ftr_list))):
            ...
            if last_feature is not None:
                feature = self.skip_add.add(feature, F.interpolate(last_feature, scale_factor=2.0, mode='nearest'))
            # smooth
    ...
    def forward(self, x, x_edge, x_warps, x_conds, warp_feature=True):
        ...
        for i in range(len(x_warps)):
              ...
              if last_flow is not None and warp_feature:
                  x_warp_after = F.grid_sample(self.dequant(x_warp), self.dequant(last_flow).detach().permute(0, 2, 3, 1),
                       mode='bilinear', padding_mode='border')
                  x_warp_after = self.quant(x_warp_after)
              else:
                  x_warp_after = x_warp

              kernel = CorrTorch(max_disp=3, dila_patch=1)
            
              corre_out = self.quant(kernel(self.dequant(x_warp_after), self.dequant(x_cond)))
              tenCorrelation = F.leaky_relu(input=corre_out, negative_slope=0.1, inplace=False)
              
              flow = self.netMain[i](tenCorrelation)
              delta_list.append(flow)
              flow = self.quant(apply_offset(self.dequant(flow)))

              if last_flow is not None:
                  flow = F.grid_sample(self.dequant(last_flow), self.dequant(flow), mode='bilinear', padding_mode='border')
                  flow = self.quant(flow)
              else:
                  flow = flow.permute(0, 3, 1, 2)

              last_flow = flow
              x_warp = F.grid_sample(self.dequant(x_warp), self.dequant(flow).permute(0, 2, 3, 1),mode='bilinear', padding_mode='border')
              x_warp = self.quant(x_warp)
              concat = torch.cat([x_warp,x_cond],1)
              flow = self.netRefine[i](concat)
              delta_list.append(flow)
              flow = apply_offset(self.dequant(flow))
              flow = self.quant(F.grid_sample(self.dequant(last_flow), flow, mode='bilinear', padding_mode='border'))         
              ...
              cur_x_warp = self.quant(F.grid_sample(self.dequant(cur_x), self.dequant(last_flow).permute(0, 2, 3, 1),mode='bilinear', padding_mode='border'))
              x_all.append(cur_x_warp)
              cur_x_edge = F.interpolate(x_edge, scale_factor=0.5**(len(x_warps)-1-i), mode='bilinear')
              cur_x_warp_edge = self.quant(F.grid_sample(self.dequant(cur_x_edge), self.dequant(last_flow).permute(0, 2, 3, 1),mode='bilinear', padding_mode='zeros'))
        ...
        x_warp = F.grid_sample(self.dequant(x), self.dequant(last_flow).permute(0, 2, 3, 1),
                     mode='bilinear', padding_mode='border')
        x_warp = self.quant(x_warp)
        return x_warp, last_flow, cond_fea_all, last_flow_all, delta_list, x_all, x_edge_all, delta_x_all, delta_y_all
    
class AFWM_Quantized(nn.Module):
    def __init__(self, opt, input_nc):
        super(AFWM_Quantized, self).__init__()
...
class QuantizedModel(nn.Module):
    ...
    def forward(self, cond_input, image_input, image_edge):
        cond_input, image_input, image_edge = self.quant(cond_input), self.quant(image_input), self.quant(image_edge)
        
        x_warp, last_flow, last_flow_all, flow_all, delta_list, x_all, x_edge_all, delta_x_all, delta_y_all = \
            self.model_fp32(cond_input, image_input, image_edge)

        return self.dequant(x_warp), self.dequant(last_flow), self.dequant(last_flow_all), self.dequant(flow_all), \
                self.dequant(delta_list), self.dequant(x_all), self.dequant(x_edge_all), self.dequant(delta_x_all), self.dequant(delta_y_all)