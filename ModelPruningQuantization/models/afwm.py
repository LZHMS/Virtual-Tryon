...
class FP32NonTraceable(nn.Module):
    ...
    def forward(self, x):
        return F.leaky_relu(input=x, negative_slope=0.1, inplace=False)
...
class AFlowNet(nn.Module):
    ...
    def forward(self, x, x_edge, x_warps, x_conds, warp_feature=True):
        ...
        for i in range(len(x_warps)):
              ...
              kernelOut = kernel(x_warp_after, x_cond)
    
              flow = self.netMain[i](self.non_traceable_submodule(kernelOut))
